...
mgmt_cli -r true add simple-cluster name "${CLUSTER_NAME}"\
    ...
    interfaces.1.ip-address "${MAIN_IP}"\
    ...
    interfaces.2.interface-type "cluster + sync"\
    interfaces.2.ip-address "${ETH1_VIP}"\
    interfaces.2.network-mask "${BACK_NETMASK}"\
    ...
    members.1.name "${CLUSTER_NAME}1"\
    members.1.one-time-password "${SIC_KEY}"\
    members.1.ip-address "${NODE1_PUB_PIP}"\
    ...
    members.2.name "${CLUSTER_NAME}2"\
    members.2.one-time-password "${SIC_KEY}"\
    members.2.ip-address "${NODE2_PUB_PIP}"\
