...
resource "checkpoint_management_access_rule" "vnet_egress" {
  ...
  source      = [checkpoint_management_network.remote_vnet.id]
  destination = ["Any"]
  service     = ["Any"]
  content     = ["Any"]
  time        = ["Any"]
...
resource "checkpoint_management_access_rule" "ssh_cluster_nodes" {
  name        = "cluster nodes management SSH"
  layer       = local.layer_name
...
# allow HTTP/HTTPS to nodes from my IP
resource "checkpoint_management_access_rule" "http_https_cluster_nodes" {
  name        = "cluster nodes management HTTP/HTTPS"
  layer       = local.layer_name
  position    = { above = checkpoint_management_access_rule.vnet_egress.id }
  source      = ["myip"]
  destination = ["Any"] // ["ha1", "ha2"]
